...
class VerticalTopBottomDetector(BaseDetector):
    ...
    def __init__(self, model_path='yolov8l.pt'):
        ...
        print(f"\n{'='*70}")
        print(f"🚦 VERTICAL TOP→BOTTOM DIRECTIONAL DETECTOR")
        print(f"{'='*70}")
        ...
        self.direction_name = "Vertical Top→Bottom"
        self.valid_direction = (0, 1)  # Moving downward (positive Y)
        
        print(f"\n🎯 Configuration:")
        print(f"   Direction: {self.direction_name}")
        print(f"   Counting line: Bottom 1/3 of frame")
        print(f"   Valid movement: Downward (Y increasing)")
    ...
    def is_valid_direction(self, track_history, valid_direction_vector):
        ...
        # Check Y movement (should be increasing = moving down)
        ...
        # Must have majority of movements downward (positive change)
        downward_moves = sum(1 for change in y_changes if change > 2)
        upward_moves = sum(1 for change in y_changes if change < -2)
        
        # More downward than upward movement
        return downward_moves > upward_moves and downward_moves >= len(y_changes) * 0.5
    
    def check_line_crossing(self, prev_point, current_point):
        """Check if vehicle crossed the counting line (downward only)"""
        ...
        # AND vehicle is moving downward (y2 > y1)
    ...
    def draw_detections(self, frame, detections, congestion_info, fps):
        ...
        # Header
        cv2.putText(frame, "VERTICAL TOP→BOTTOM DETECTOR", (20, 40),
        ...
        # Draw downward arrows along line
