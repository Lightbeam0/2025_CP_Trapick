...
class HorizontalRightLeftDetector(BaseDetector):
    ...
    def __init__(self, model_path='yolov8l.pt'):
        ...
        print(f"\n{'='*70}")
        print(f"🚦 HORIZONTAL RIGHT→LEFT DIRECTIONAL DETECTOR")
        print(f"{'='*70}")
        ...
        self.direction_name = "Horizontal Right→Left"
        self.valid_direction = (-1, 0)  # Moving leftward (negative X)
        
        print(f"\n🎯 Configuration:")
        print(f"   Direction: {self.direction_name}")
        print(f"   Counting line: Left 1/3 of frame")
        print(f"   Valid movement: Leftward (X decreasing)")
    ...
    def is_valid_direction(self, track_history, valid_direction_vector):
        ...
        # Check X movement (should be decreasing = moving left)
        ...
        # Must have majority of movements leftward (negative change)
        leftward_moves = sum(1 for change in x_changes if change < -2)
        rightward_moves = sum(1 for change in x_changes if change > 2)
        
        # More leftward than rightward movement
        return leftward_moves > rightward_moves and leftward_moves >= len(x_changes) * 0.5
    
    def check_line_crossing(self, prev_point, current_point):
        """Check if vehicle crossed the counting line (leftward only)"""
        ...
        # AND vehicle is moving leftward (x2 < x1)
            ...
            for box, tid, cid, conf in zip(boxes, ids, cls, confs):
                ...
                if (status['valid_direction'] and not status['crossed'] and 
                    ...
                    if self.check_line_crossing(prev_position, (cx, cy)):
                        ...
                        print(f"✓ #{self.total_count:03d} {name.upper()} ID:{tid} COUNTED (RIGHT→LEFT)")
    ...
    def draw_detections(self, frame, detections, congestion_info, fps):
        ...
        # Header
        cv2.putText(frame, "HORIZONTAL RIGHT→LEFT DETECTOR", (20, 40),
        ...
        # Draw leftward arrows along line
        line_length = self.line_end[1] - self.line_start[1]
        if line_length > 0:
            # Draw multiple leftward arrows
        ...
        # Add direction label
        cv2.putText(frame, "COUNTING LINE (RIGHT→LEFT)", 
